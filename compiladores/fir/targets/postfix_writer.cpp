...
void fir::postfix_writer::do_iterate_node(fir::iterate_node *const node, int lvl) {
  ASSERT_SAFE_EXPRESSIONS;
  ...
  for(int i = 0; i < node->count(); i++) {
    ...
    //do_index_node(new fir::index_node(lvl, vetor, new cdk::expression_node(lvl, i));

    do_function_call_node(new fir::function_call_node(lvl, node->identifier(), new cdk::sequence_node(lvl, node->vetor())));
